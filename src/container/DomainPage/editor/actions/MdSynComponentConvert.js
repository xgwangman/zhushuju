...
import {Modal, Form, Input, Button, Row, Select,Table, Col, Icon, Switch, Tooltip} from 'antd';
const Option = Select.Option;
let uuid = 0;

/* 主数据同步 */
class MdSynComponentConvert extends Component {
    constructor(props) {
        super(props);
        //this.props.requestEntityAttributes();
    ...
    handleSubmit=(e)=>{
        let {show, componentData, entityId, entityAttrList, flowFields, onCloseWindow} = this.props;
        e.preventDefault();
            ...
            if (!err) {
                ...
                if (editor) {
                    ...
                    fieldList.map(function (item,index) {
                        ...
                        field.srcDisplayName =item.displayName;
    ...
    render() {
        let {show, componentData, entityAttrList, flowFields, onCloseWindow} = this.props;
        ...
        let columns=[
            ...
            {title: '转换描述', dataIndex: 'description',width: '20%',render: (text, record, index)=> {
                return <Input value={this.state.fieldList.length>0?this.state.fieldList[index]['description']:text} type='input'
            ...
            >
                <Form>
                    ...
                    <FormItem {...formItemLayout} label="字段映射">
                        <Button type="primary" icon="sync" size="small"
                                onClick={(e)=>this.requestFlowField(componentData.id)}>获取字段</Button>
                    </FormItem>
                    <div>
    ...
    requestTableDate=(isShow)=>{
        ...
        if(isShow&&componentData&&componentData.fieldMapping&&componentData.fieldMapping.length>0&&this.initStatus){
            ...
            let field =[];
            let fieldMapping =componentData.fieldMapping;
            fieldMapping.map(function (item,index) {
                let temp={};
                temp.tableName=item.srcTableName;
                temp.id=item.srcFieldId;
                temp.name=item.srcFieldName;
                temp.displayName=item.srcDisplayName;
                temp.attId=item.attId;
                temp.attName=item.attName;
                temp.attType=item.attType;
                temp.attColumnName=item.attColumnName;
                temp.attLength=item.attLength;
                temp.aliasName=item.attAlias;
                temp.isPk=item.isPk;
                temp.description=item.tranRemark;
                field.push(temp);
            });
            this.setState({selectedFields:[...allFields],allFieldList:[...allFields],fieldList:field});
    ...
    requestFlowField=(id)=>{
        //selectedFields 选择框中的字段 ，allFieldList 表格中所有的字段，fieldList 表格显示的字段
...
export default Form.create()(MdSynComponentConvert);