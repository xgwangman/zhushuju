...
class JdbcQuerySqlComponentConvert extends Component {
                ...
                <Form>
                    ...
                    <FormItem {...formItemLayout} label="数据源">
                        {getFieldDecorator('dataSrc', {
                            ...
                            <Select size={'large'} placeholder="数据源名称" onSelect={(id)=> {
                                ...
                                {(taskAttr.dataSourceList || []).map((item, idx)=><Option
                                    key={"datasrc" + idx}
                    ...
                    <FormItem {...formItemLayout} label="sql编辑">
                        {getFieldDecorator('codeEdit', {
                            ...
                            <CodeMirror
                                sql={componentData ? (componentData.pubTableMeta ? componentData.pubTableMeta.sql : '') : ''}/>
                        )}
                    </FormItem>
    ...
    onSelectToCacheData = (value, type)=> {
        if (type == "dataSource") {
            ...
            window.codeEditor.setValue("SELECT * FROM " + this.state.table.name);//选中表后，给sql编辑器设定一个默认sql
